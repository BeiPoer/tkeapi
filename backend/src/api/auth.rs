...
mod tests {
    ...
    #[test]
    fn test_check_ip_blacklist_single_ip() {
        let settings = SiteSettings {
            ip_blacklist_enabled: true,
            ip_blacklist: vec!["192.168.1.100".to_string(), "10.0.0.1".to_string()],
            ..Default::default()
        };

        assert!(check_ip_blacklist(&settings, "192.168.1.100").is_err());
        assert!(check_ip_blacklist(&settings, "10.0.0.1").is_err());
        assert!(check_ip_blacklist(&settings, "192.168.1.101").is_ok());
    }

    #[test]
    fn test_check_ip_blacklist_cidr() {
        let settings = SiteSettings {
            ip_blacklist_enabled: true,
            ip_blacklist: vec!["192.168.1.0/24".to_string(), "10.0.0.0/8".to_string()],
            ..Default::default()
        };

        assert!(check_ip_blacklist(&settings, "192.168.1.1").is_err());
        assert!(check_ip_blacklist(&settings, "192.168.1.255").is_err());
        assert!(check_ip_blacklist(&settings, "10.254.254.254").is_err());
        assert!(check_ip_blacklist(&settings, "192.168.2.1").is_ok());
    }

    #[test]
    fn test_check_ip_blacklist_ipv4_mapped() {
        let settings = SiteSettings {
            ip_blacklist_enabled: true,
        ...
        assert!(check_ip_blacklist(&settings, "::ffff:192.168.1.100").is_err());
    }

    #[test]
    fn test_check_ip_blacklist_disabled() {
        let settings = SiteSettings {
        ...
        assert!(check_ip_blacklist(&settings, "192.168.1.100").is_ok());
    }
}

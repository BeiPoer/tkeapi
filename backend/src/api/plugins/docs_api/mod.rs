...
static CHAT_ZH: LazyLock<String> = LazyLock::new(|| {
    format!(
        "{}\n\n{}",
        include_str!("default_docs/zh/chat.md"),
        include_str!("default_docs/zh/common-errors.md")
    )
});
static CHAT_ANTHROPIC_ZH: LazyLock<String> = LazyLock::new(|| {
    format!(
        "{}\n\n{}",
        include_str!("default_docs/zh/chat-anthropic.md"),
        include_str!("default_docs/zh/common-errors.md")
    )
});
...
static MINIMAX_VID_ZH: LazyLock<String> = LazyLock::new(|| {
    format!(
        "{}\n\n{}",
        include_str!("default_docs/zh/minimax-video.md"),
        include_str!("default_docs/zh/common-errors.md")
    )
});
use crate::{
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/docs/clear-all", post(clear_all_docs))
    ...
    let docs: Vec<PluginDoc> = if let Some(ref l) = lang {
        sqlx::query_as(
            &state.db.format_query(
                "SELECT p.id, p.parent_id, p.is_dir, p.sort_order, p.is_active, p.slug, p.created_at, p.updated_at, p.category_id, \
...
async fn create_doc(
    ...
    let doc: PluginDoc = sqlx::query_as(
        &state.db.format_query(
            "INSERT INTO plugin_docs (parent_id, title, content, is_dir, sort_order, is_active, slug, category_id, created_at, updated_at) \
...
async fn update_doc(
    ...
    let updated: PluginDoc = sqlx::query_as(
        &state.db.format_query(
            "UPDATE plugin_docs SET parent_id = ?, title = ?, content = ?, sort_order = ?, is_active = ?, slug = ?, category_id = ?, updated_at = CURRENT_TIMESTAMP \
             WHERE id = ? RETURNING *"
        )
    )
    .bind(payload.parent_id)
...
async fn translate_content(
    ...
    let resp = crate::services::http_client::with_upstream_timeout(
        state
            .http_client
            .post(&api_url)
...
pub async fn seed_default_docs_direct(pool: &Pool<Postgres>) -> Result<(), sqlx::Error> {
    ...
    for (cat_title, cat_slug, cat_order, articles) in default_data {
        ...
        // 插入目录的翻译
        let cat_translations = default_docs::get_category_translations(cat_slug);
        ...
        for (art_title, art_slug, art_order, art_content) in articles {
            ...
            // 插入文章的翻译
            let art_translations = default_docs::get_article_translations(art_slug);
...
pub fn get_default_docs_data() -> Vec<(
    &'static str,
    &'static str,
    i32,
    ...
    vec![
        ...
        (
            ...
            15,
            vec![
                ("聊天 Chat", "chat", 10, &*CHAT_ZH),
                ("聊天 Anthropic", "chat-anthropic", 15, &*CHAT_ANTHROPIC_ZH),
                ("gpt-image-2 图像生成", "gpt-image", 20, &*GPT_IMG_ZH),
                ("gemini-3.1 图像生成", "google-image", 30, &*GOOGLE_IMG_ZH),
                ("doubao-seedream 图像生成", "volc-image", 40, &*VOLC_IMG_ZH),
...
支持传入底图（Image）与遮罩蒙版（Mask），配合提示词进行局部擦除与重绘。兼容以下两种传参方式：
...
可通过 `image` 或 `image_urls` 传入参考图做图生图 / 多图参考（`role` 与 `type` 等价）：
...
OpenAI 兼容异步视频接口 `/v1/video/generations`。多数视频模型为异步：先**提交任务**，再**轮询结果**。
...
按上方角色约定传入即可。"#
        ...
        (
            ...
            90,
            vec![(
                "Codex与Claude Code接入指南",
                "codex-claude-guide",
                10,
                include_str!("default_docs/zh/codex-claude-guide.md"),
            )],
        )
    ]
}

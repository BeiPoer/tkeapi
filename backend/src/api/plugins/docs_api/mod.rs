...
| `response_format` | `string` | 否 | **投递方式**：`url`（返回图片链接，默认）或 `b64_json`（返回 Base64）。与图片编码格式无关 |
| `output_format` | `string` | 否 | **图片编码格式**（如 `png` / `jpeg` / `webp`）。火山方舟原生透传；腾讯云 VOD 会映射为 `OutputConfig.OutputFormat` |
